...
# preallocate
drug_tree_metrics_avg = [0,0,0,0,0]
drug_tree_reduced_metrics_avg = [0,0,0,0,0]
iris_tree_metrics_avg = [0,0,0,0,0]
iris_tree_reduced_metrics_avg = [0,0,0,0,0]
seeds_tree_metrics_avg = [0,0,0,0,0]
seeds_tree_reduced_metrics_avg = [0,0,0,0,0]
drug_svm_metrics_avg = [0,0,0,0,0]
drug_svm_reduced_metrics_avg = [0,0,0,0,0]
iris_svm_metrics_avg = [0,0,0,0,0]
iris_svm_reduced_metrics_avg = [0,0,0,0,0]
seeds_svm_metrics_avg = [0,0,0,0,0]
seeds_svm_reduced_metrics_avg = [0,0,0,0,0]
drug_gp_metrics_avg = [0,0,0,0,0]
drug_gp_reduced_metrics_avg = [0,0,0,0,0]
iris_gp_metrics_avg = [0,0,0,0,0]
iris_gp_reduced_metrics_avg = [0,0,0,0,0]
seeds_gp_metrics_avg = [0,0,0,0,0]
seeds_gp_reduced_metrics_avg = [0,0,0,0,0]
drug_knn_metrics_avg = [0,0,0,0,0]
drug_knn_reduced_metrics_avg = [0,0,0,0,0]
iris_knn_metrics_avg = [0,0,0,0,0]
iris_knn_reduced_metrics_avg = [0,0,0,0,0]
seeds_knn_metrics_avg = [0,0,0,0,0]
seeds_knn_reduced_metrics_avg = [0,0,0,0,0]
drug_mlp_metrics_avg = [0,0,0,0,0]
drug_mlp_reduced_metrics_avg = [0,0,0,0,0]
iris_mlp_metrics_avg = [0,0,0,0,0]
iris_mlp_reduced_metrics_avg = [0,0,0,0,0]
seeds_mlp_metrics_avg = [0,0,0,0,0]
seeds_mlp_reduced_metrics_avg = [0,0,0,0,0]
...
# print("KNN:              ", mod.testModel(seeds_knn_reduced, seedsX_test_reduced, seedsY_test_reduced))
    ...
    drug_tree_reduced_metrics = per.getMetrics(drug_tree_reduced,drugX_test_reduced,drugY_test_reduced)
    iris_tree_metrics = per.getMetrics(iris_tree,irisX_test,irisY_test)
    iris_tree_reduced_metrics = per.getMetrics(iris_tree_reduced,irisX_test_reduced,irisY_test_reduced)
    seeds_tree_metrics = per.getMetrics(seeds_tree,seedsX_test,seedsY_test)
    seeds_tree_reduced_metrics = per.getMetrics(seeds_tree_reduced,seedsX_test_reduced,seedsY_test_reduced)

    # SVM
    drug_svm_metrics = per.getMetrics(drug_svm,drugX_test,drugY_test)
    drug_svm_reduced_metrics = per.getMetrics(drug_svm_reduced,drugX_test_reduced,drugY_test_reduced)
    iris_svm_metrics = per.getMetrics(iris_svm,irisX_test,irisY_test)
    iris_svm_reduced_metrics = per.getMetrics(iris_svm_reduced,irisX_test_reduced,irisY_test_reduced)
    seeds_svm_metrics = per.getMetrics(seeds_svm,seedsX_test,seedsY_test)
    seeds_svm_reduced_metrics = per.getMetrics(seeds_svm_reduced,seedsX_test_reduced,seedsY_test_reduced)

    # Gaussian Process
    drug_gp_metrics = per.getMetrics(drug_gp,drugX_test,drugY_test)
    drug_gp_reduced_metrics = per.getMetrics(drug_gp_reduced,drugX_test_reduced,drugY_test_reduced)
    iris_gp_metrics = per.getMetrics(iris_gp,irisX_test,irisY_test)
    iris_gp_reduced_metrics = per.getMetrics(iris_gp_reduced,irisX_test_reduced,irisY_test_reduced)
    seeds_gp_metrics = per.getMetrics(seeds_gp,seedsX_test,seedsY_test)
    seeds_gp_reduced_metrics = per.getMetrics(seeds_gp_reduced,seedsX_test_reduced,seedsY_test_reduced)

    # KNN
    drug_knn_metrics = per.getMetrics(drug_knn,drugX_test,drugY_test)
    drug_knn_reduced_metrics = per.getMetrics(drug_knn_reduced,drugX_test_reduced,drugY_test_reduced)
    iris_knn_metrics = per.getMetrics(iris_knn,irisX_test,irisY_test)
    iris_knn_reduced_metrics = per.getMetrics(iris_knn_reduced,irisX_test_reduced,irisY_test_reduced)
    seeds_knn_metrics = per.getMetrics(seeds_knn,seedsX_test,seedsY_test)
    seeds_knn_reduced_metrics = per.getMetrics(seeds_knn_reduced,seedsX_test_reduced,seedsY_test_reduced)

    # MLP
    drug_mlp_metrics = per.getMetrics(drug_mlp,drugX_test,drugY_test)
    drug_mlp_reduced_metrics = per.getMetrics(drug_mlp_reduced,drugX_test_reduced,drugY_test_reduced)
    iris_mlp_metrics = per.getMetrics(iris_mlp,irisX_test,irisY_test)
    iris_mlp_reduced_metrics = per.getMetrics(iris_mlp_reduced,irisX_test_reduced,irisY_test_reduced)
    seeds_mlp_metrics = per.getMetrics(seeds_mlp,seedsX_test,seedsY_test)
    seeds_mlp_reduced_metrics = per.getMetrics(seeds_mlp_reduced,seedsX_test_reduced,seedsY_test_reduced)

    # throw out runs with accuracy == 1
    if drug_tree_metrics[0] == 1 or \
       iris_tree_metrics[0] == 1 or \
       seeds_tree_metrics[0] == 1 or \
       drug_svm_metrics[0] == 1 or \
       iris_svm_metrics[0] == 1 or \
       seeds_svm_metrics[0] == 1 or \
       drug_gp_metrics[0] == 1 or \
       iris_gp_metrics[0] == 1 or \
       seeds_gp_metrics[0] == 1 or \
       drug_knn_metrics[0] == 1 or \
       iris_knn_metrics[0] == 1 or \
       seeds_knn_metrics[0] == 1 or \
       drug_mlp_metrics[0] == 1 or \
       iris_mlp_metrics[0] == 1 or \
       seeds_mlp_metrics[0] == 1:
    ...
    # running total
    drug_tree_metrics_avg = [sum(x) for x in zip(drug_tree_metrics_avg,drug_tree_metrics)]
    drug_tree_reduced_metrics_avg = [sum(x) for x in zip(drug_tree_reduced_metrics_avg,drug_tree_reduced_metrics)]
    iris_tree_metrics_avg = [sum(x) for x in zip(iris_tree_metrics_avg,iris_tree_metrics)]
    iris_tree_reduced_metrics_avg = [sum(x) for x in zip(iris_tree_reduced_metrics_avg,iris_tree_reduced_metrics)]
    seeds_tree_metrics_avg = [sum(x) for x in zip(seeds_tree_metrics_avg,seeds_tree_metrics)]
    seeds_tree_reduced_metrics_avg = [sum(x) for x in zip(seeds_tree_reduced_metrics_avg,seeds_tree_reduced_metrics)]
    drug_svm_metrics_avg = [sum(x) for x in zip(drug_svm_metrics_avg,drug_svm_metrics)]
    drug_svm_reduced_metrics_avg = [sum(x) for x in zip(drug_svm_reduced_metrics_avg,drug_svm_reduced_metrics)]
    iris_svm_metrics_avg = [sum(x) for x in zip(iris_svm_metrics_avg,iris_svm_metrics)]
    iris_svm_reduced_metrics_avg = [sum(x) for x in zip(iris_svm_reduced_metrics_avg,iris_svm_reduced_metrics)]
    seeds_svm_metrics_avg = [sum(x) for x in zip(seeds_svm_metrics_avg,seeds_svm_metrics)]
    seeds_svm_reduced_metrics_avg = [sum(x) for x in zip(seeds_svm_reduced_metrics_avg,seeds_svm_reduced_metrics)]
    drug_gp_metrics_avg = [sum(x) for x in zip(drug_gp_metrics_avg,drug_gp_metrics)]
    drug_gp_reduced_metrics_avg = [sum(x) for x in zip(drug_gp_reduced_metrics_avg,drug_gp_reduced_metrics)]
    iris_gp_metrics_avg = [sum(x) for x in zip(iris_gp_metrics_avg,iris_gp_metrics)]
    iris_gp_reduced_metrics_avg = [sum(x) for x in zip(iris_gp_reduced_metrics_avg,iris_gp_reduced_metrics)]
    seeds_gp_metrics_avg = [sum(x) for x in zip(seeds_gp_metrics_avg,seeds_gp_metrics)]
    seeds_gp_reduced_metrics_avg = [sum(x) for x in zip(seeds_gp_reduced_metrics_avg,seeds_gp_reduced_metrics)]
    drug_knn_metrics_avg = [sum(x) for x in zip(drug_knn_metrics_avg,drug_knn_metrics)]
    drug_knn_reduced_metrics_avg = [sum(x) for x in zip(drug_knn_reduced_metrics_avg,drug_knn_reduced_metrics)]
    iris_knn_metrics_avg = [sum(x) for x in zip(iris_knn_metrics_avg,iris_knn_metrics)]
    iris_knn_reduced_metrics_avg = [sum(x) for x in zip(iris_knn_reduced_metrics_avg,iris_knn_reduced_metrics)]
    seeds_knn_metrics_avg = [sum(x) for x in zip(seeds_knn_metrics_avg,seeds_knn_metrics)]
    seeds_knn_reduced_metrics_avg = [sum(x) for x in zip(seeds_knn_reduced_metrics_avg,seeds_knn_reduced_metrics)]
    drug_mlp_metrics_avg = [sum(x) for x in zip(drug_mlp_metrics_avg,drug_mlp_metrics)]
    drug_mlp_reduced_metrics_avg = [sum(x) for x in zip(drug_mlp_reduced_metrics_avg,drug_mlp_reduced_metrics)]
    iris_mlp_metrics_avg = [sum(x) for x in zip(iris_mlp_metrics_avg,iris_mlp_metrics)]
    iris_mlp_reduced_metrics_avg = [sum(x) for x in zip(iris_mlp_reduced_metrics_avg,iris_mlp_reduced_metrics)]
    seeds_mlp_metrics_avg = [sum(x) for x in zip(seeds_mlp_metrics_avg,seeds_mlp_metrics)]
    seeds_mlp_reduced_metrics_avg = [sum(x) for x in zip(seeds_mlp_reduced_metrics_avg,seeds_mlp_reduced_metrics)]

# average
drug_tree_metrics_avg = [x/(averages-counter) for x in drug_tree_metrics_avg]
drug_tree_reduced_metrics_avg = [x/(averages-counter) for x in drug_tree_reduced_metrics_avg]
iris_tree_metrics_avg = [x/(averages-counter) for x in iris_tree_metrics_avg]
iris_tree_reduced_metrics_avg = [x/(averages-counter) for x in iris_tree_reduced_metrics_avg]
seeds_tree_metrics_avg = [x/(averages-counter) for x in seeds_tree_metrics_avg]
seeds_tree_reduced_metrics_avg = [x/(averages-counter) for x in seeds_tree_reduced_metrics_avg]
drug_svm_metrics_avg = [x/(averages-counter) for x in drug_svm_metrics_avg]
drug_svm_reduced_metrics_avg = [x/(averages-counter) for x in drug_svm_reduced_metrics_avg]
iris_svm_metrics_avg = [x/(averages-counter) for x in iris_svm_metrics_avg]
iris_svm_reduced_metrics_avg = [x/(averages-counter) for x in iris_svm_reduced_metrics_avg]
seeds_svm_metrics_avg = [x/(averages-counter) for x in seeds_svm_metrics_avg]
seeds_svm_reduced_metrics_avg = [x/(averages-counter) for x in seeds_svm_reduced_metrics_avg]
drug_gp_metrics_avg = [x/(averages-counter) for x in drug_gp_metrics_avg]
drug_gp_reduced_metrics_avg = [x/(averages-counter) for x in drug_gp_reduced_metrics_avg]
iris_gp_metrics_avg = [x/(averages-counter) for x in iris_gp_metrics_avg]
iris_gp_reduced_metrics_avg = [x/(averages-counter) for x in iris_gp_reduced_metrics_avg]
seeds_gp_metrics_avg = [x/(averages-counter) for x in seeds_gp_metrics_avg]
seeds_gp_reduced_metrics_avg = [x/(averages-counter) for x in seeds_gp_reduced_metrics_avg]
drug_knn_metrics_avg = [x/(averages-counter) for x in drug_knn_metrics_avg]
drug_knn_reduced_metrics_avg = [x/(averages-counter) for x in drug_knn_reduced_metrics_avg]
iris_knn_metrics_avg = [x/(averages-counter) for x in iris_knn_metrics_avg]
iris_knn_reduced_metrics_avg = [x/(averages-counter) for x in iris_knn_reduced_metrics_avg]
seeds_knn_metrics_avg = [x/(averages-counter) for x in seeds_knn_metrics_avg]
seeds_knn_reduced_metrics_avg = [x/(averages-counter) for x in seeds_knn_reduced_metrics_avg]
drug_mlp_metrics_avg = [x/(averages-counter) for x in drug_mlp_metrics_avg]
drug_mlp_reduced_metrics_avg = [x/(averages-counter) for x in drug_mlp_reduced_metrics_avg]
iris_mlp_metrics_avg = [x/(averages-counter) for x in iris_mlp_metrics_avg]
iris_mlp_reduced_metrics_avg = [x/(averages-counter) for x in iris_mlp_reduced_metrics_avg]
seeds_mlp_metrics_avg = [x/(averages-counter) for x in seeds_mlp_metrics_avg]
seeds_mlp_reduced_metrics_avg = [x/(averages-counter) for x in seeds_mlp_reduced_metrics_avg]

# combine
drug_metrics = [drug_tree_metrics_avg,drug_svm_metrics_avg,drug_gp_metrics_avg,drug_knn_metrics_avg, drug_mlp_metrics_avg]
drug_reduced_metrics = [drug_tree_reduced_metrics_avg,drug_svm_reduced_metrics_avg,drug_gp_reduced_metrics_avg,drug_knn_reduced_metrics_avg, drug_mlp_reduced_metrics_avg]
iris_metrics = [iris_tree_metrics_avg,iris_svm_metrics_avg,iris_gp_metrics_avg,iris_knn_metrics_avg, iris_mlp_metrics_avg]
iris_reduced_metrics = [iris_tree_reduced_metrics_avg,iris_svm_reduced_metrics_avg,iris_gp_reduced_metrics_avg,iris_knn_reduced_metrics_avg, iris_mlp_reduced_metrics_avg]
seeds_metrics = [seeds_tree_metrics_avg,seeds_svm_metrics_avg,seeds_gp_metrics_avg,seeds_knn_metrics_avg, seeds_mlp_metrics_avg]
seeds_reduced_metrics = [seeds_tree_reduced_metrics_avg,seeds_svm_reduced_metrics_avg,seeds_gp_reduced_metrics_avg,seeds_knn_reduced_metrics_avg, seeds_mlp_reduced_metrics_avg]

print('\n')
per.printMetrics('drug',drug_metrics)
print('\n')
per.printMetrics('reduced drug',drug_reduced_metrics)
print('\n')
per.printMetrics('iris',iris_metrics)
print('\n')
per.printMetrics('reduced iris',iris_reduced_metrics)
print('\n')
per.printMetrics('seeds',seeds_metrics)
print('\n')
per.printMetrics('reduced seeds',seeds_reduced_metrics)

# save (to results/metrics)
per.exportMetrics(drug_metrics,'drug_metrics.csv')
per.exportMetrics(drug_reduced_metrics,'drug_reduced_metrics.csv')
per.exportMetrics(iris_metrics,'iris_metrics.csv')
per.exportMetrics(iris_reduced_metrics,'iris_reduced_metrics.csv')
per.exportMetrics(seeds_metrics,'seeds_metrics.csv')
per.exportMetrics(seeds_reduced_metrics,'seeds_reduced_metrics.csv')
