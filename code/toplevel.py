...
# Normal Datasets
drugX_train, drugX_test, drugY_train, drugY_test = mod.train_test_split(drug200_dataset_numpy, drug200_dataset_numpy_labels, test_size=0.25, random_state=random_state)
irisX_train, irisX_test, irisY_train, irisY_test = mod.train_test_split(iris_dataset_numpy, iris_dataset_numpy_labels, test_size=0.25, random_state=random_state)
seedsX_train, seedsX_test, seedsY_train, seedsY_test = mod.train_test_split(wheatseeds_dataset_numpy, wheatseeds_dataset_numpy_labels, test_size=0.25, random_state=random_state)
#Reduced Datasets
drugX_train_reduced, drugX_test_reduced, drugY_train_reduced, drugY_test_reduced = mod.train_test_split(drug200_dataset_reduced, drug200_dataset_numpy_labels, test_size=0.25, random_state=random_state)
irisX_train_reduced, irisX_test_reduced, irisY_train_reduced, irisY_test_reduced = mod.train_test_split(iris_dataset_reduced, iris_dataset_numpy_labels, test_size=0.25, random_state=random_state)
seedsX_train_reduced, seedsX_test_reduced, seedsY_train_reduced, seedsY_test_reduced = mod.train_test_split(wheatseeds_dataset_reduced, wheatseeds_dataset_numpy_labels, test_size=0.25, random_state=random_state)
...
# Tuning
drug_tree_params = mod.DecisionTreeTuning(drugX_train, drugY_train, drugX_test, drugY_test, random_state)
drug_knn_params = mod.KNNTuning(drugX_train, drugY_train, drugX_test, drugY_test)
drug_gp_params = mod.gpTuning(drugX_train, drugY_train, drugX_test, drugY_test, random_state)
drug_mlp_params = mod.MLPtuning(drugX_train, drugY_train, drugX_test, drugY_test, random_state)
iris_tree_params = mod.DecisionTreeTuning(irisX_train, irisY_train, irisX_test, irisY_test, random_state)
iris_knn_params = mod.KNNTuning(irisX_train, irisY_train, irisX_test, irisY_test)
iris_gp_params = mod.gpTuning(irisX_train, irisY_train, irisX_test, irisY_test, random_state)
iris_mlp_params = mod.MLPtuning(irisX_train, irisY_train, irisX_test, irisY_test, random_state)
seeds_tree_params = mod.DecisionTreeTuning(seedsX_train, seedsY_train, seedsX_test, seedsY_test, random_state)
seeds_knn_params = mod.KNNTuning(seedsX_train, seedsY_train, seedsX_test, seedsY_test)
seeds_gp_params = mod.gpTuning(seedsX_train, seedsY_train, seedsX_test, seedsY_test, random_state)
seeds_mlp_params = mod.MLPtuning(seedsX_train, seedsY_train, seedsX_test, seedsY_test, random_state)

red_drug_tree_params = mod.DecisionTreeTuning(drugX_train_reduced, drugY_train_reduced, drugX_test_reduced, drugY_test_reduced, random_state)
red_drug_knn_params = mod.KNNTuning(drugX_train_reduced, drugY_train_reduced, drugX_test_reduced, drugY_test_reduced)
red_drug_gp_params = mod.gpTuning(drugX_train_reduced, drugY_train_reduced, drugX_test_reduced, drugY_test_reduced, random_state)
red_drug_mlp_params = mod.MLPtuning(drugX_train_reduced, drugY_train_reduced, drugX_test_reduced, drugY_test_reduced, random_state)
red_iris_tree_params = mod.DecisionTreeTuning(irisX_train_reduced, irisY_train_reduced, irisX_test_reduced, irisY_test_reduced, random_state)
red_iris_knn_params = mod.KNNTuning(irisX_train_reduced, irisY_train_reduced, irisX_test_reduced, irisY_test_reduced)
red_iris_gp_params = mod.gpTuning(irisX_train_reduced, irisY_train_reduced, irisX_test_reduced, irisY_test_reduced, random_state)
red_iris_mlp_params = mod.MLPtuning(irisX_train_reduced, irisY_train_reduced, irisX_test_reduced, irisY_test_reduced, random_state)
red_seeds_tree_params = mod.DecisionTreeTuning(seedsX_train_reduced, seedsY_train_reduced, seedsX_test_reduced, seedsY_test_reduced, random_state)
red_seeds_knn_params = mod.KNNTuning(seedsX_train_reduced, seedsY_train_reduced, seedsX_test_reduced, seedsY_test_reduced)
red_seeds_gp_params = mod.gpTuning(seedsX_train_reduced, seedsY_train_reduced, seedsX_test_reduced, seedsY_test_reduced, random_state)
red_seeds_mlp_params = mod.MLPtuning(seedsX_train_reduced, seedsY_train_reduced, seedsX_test_reduced, seedsY_test_reduced, random_state)
# Decision Trees
drug_tree = mod.applyDecisionTree(drugX_train, drugY_train, drug_tree_params['max_depth'], drug_tree_params['min_samples_split'], drug_tree_params['min_samples_leaf'], drug_tree_params['criterion'], random_state)
iris_tree = mod.applyDecisionTree(irisX_train, irisY_train, iris_tree_params['max_depth'], iris_tree_params['min_samples_split'], iris_tree_params['min_samples_leaf'], iris_tree_params['criterion'], random_state)
seeds_tree = mod.applyDecisionTree(seedsX_train, seedsY_train, seeds_tree_params['max_depth'], seeds_tree_params['min_samples_split'], seeds_tree_params['min_samples_leaf'], seeds_tree_params['criterion'], random_state)
drug_tree_reduced = mod.applyDecisionTree(drugX_train_reduced, drugY_train_reduced, red_drug_tree_params['max_depth'], red_drug_tree_params['min_samples_split'], red_drug_tree_params['min_samples_leaf'], red_drug_tree_params['criterion'], random_state)
iris_tree_reduced = mod.applyDecisionTree(irisX_train_reduced, irisY_train_reduced, red_iris_tree_params['max_depth'], red_iris_tree_params['min_samples_split'], red_iris_tree_params['min_samples_leaf'], red_iris_tree_params['criterion'], random_state)
seeds_tree_reduced = mod.applyDecisionTree(seedsX_train_reduced, seedsY_train_reduced, red_seeds_tree_params['max_depth'], red_seeds_tree_params['min_samples_split'], red_seeds_tree_params['min_samples_leaf'], red_seeds_tree_params['criterion'], random_state)

# SVM
drug_svm = mod.applyLinearSVC(drugX_train, drugY_train, random_state)
iris_svm = mod.applyLinearSVC(irisX_train, irisY_train, random_state)
seeds_svm = mod.applyLinearSVC(seedsX_train, seedsY_train, random_state)
drug_svm_reduced = mod.applyLinearSVC(drugX_train_reduced, drugY_train_reduced, random_state)
iris_svm_reduced = mod.applyLinearSVC(irisX_train_reduced, irisY_train_reduced, random_state)
seeds_svm_reduced = mod.applyLinearSVC(seedsX_train_reduced, seedsY_train_reduced, random_state)

# Gaussian Process
drug_gp = mod.applyGP(drugX_train, drugY_train, drug_gp_params['kernel'], random_state)
iris_gp = mod.applyGP(irisX_train, irisY_train, iris_gp_params['kernel'], random_state)
seeds_gp = mod.applyGP(seedsX_train, seedsY_train, seeds_gp_params['kernel'], random_state)
drug_gp_reduced = mod.applyGP(drugX_train_reduced, drugY_train_reduced, red_drug_gp_params['kernel'], random_state)
iris_gp_reduced = mod.applyGP(irisX_train_reduced, irisY_train_reduced, red_iris_gp_params['kernel'], random_state)
seeds_gp_reduced = mod.applyGP(seedsX_train_reduced, seedsY_train_reduced, red_seeds_gp_params['kernel'], random_state)
...
# MLP CLF
drug_mlp = mod.applyMLP(drugX_train, drugY_train, drug_mlp_params['alpha'], drug_mlp_params['activation'], random_state)
iris_mlp = mod.applyMLP(irisX_train, irisY_train, iris_mlp_params['alpha'], iris_mlp_params['activation'], random_state)
seeds_mlp = mod.applyMLP(seedsX_train, seedsY_train, seeds_mlp_params['alpha'], seeds_mlp_params['activation'], random_state)
drug_mlp_reduced = mod.applyMLP(drugX_train_reduced, drugY_train_reduced, red_drug_mlp_params['alpha'], red_drug_mlp_params['activation'], random_state)
iris_mlp_reduced = mod.applyMLP(irisX_train_reduced, irisY_train_reduced, red_iris_mlp_params['alpha'], red_iris_mlp_params['activation'], random_state)
seeds_mlp_reduced = mod.applyMLP(seedsX_train_reduced, seedsY_train_reduced, red_seeds_mlp_params['alpha'], red_seeds_mlp_params['activation'], random_state)

## Report the Performance of Models
print("--------------- DRUG DATA -------------------")
print("Decision Tree:    ", mod.testModel(drug_tree, drugX_test, drugY_test))
print("SVC:              ", mod.testModel(drug_svm, drugX_test, drugY_test))
...
print("SVC:              ", mod.testModel(drug_svm_reduced, drugX_test_reduced, drugY_test_reduced))
...
print("SVC:              ", mod.testModel(iris_svm, irisX_test, irisY_test))
...
print("SVC:              ", mod.testModel(iris_svm_reduced, irisX_test_reduced, irisY_test_reduced))
...
print("SVC:              ", mod.testModel(seeds_svm, seedsX_test, seedsY_test))
...
print("SVC:              ", mod.testModel(seeds_svm_reduced, seedsX_test_reduced, seedsY_test_reduced))
